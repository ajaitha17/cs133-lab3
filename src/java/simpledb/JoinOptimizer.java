...
public class JoinOptimizer {
    ...
    public double estimateJoinCost(LogicalJoinNode j, int card1, int card2,
	    ...
	    // Just finish the else case below
            ...
            double cost = cost1 + card1*cost2 + card1*cost2;
            return cost;
        }
    }
    ...
    public static int estimateTableJoinCardinality(Predicate.Op joinOp,
        ...
        // System.out.println(card1 + " " + card2 + " " + t1pkey + " " + t2pkey);
        int card = 1;
    ...
    public Vector<LogicalJoinNode> orderJoins(
        ...
        Set<Set<LogicalJoinNode>> join = enumerateSubsets(this.joins,1);
        Set<Set<LogicalJoinNode>> temp;
        int i,size = join.size(),s;

        for(i=0;i<=size;i++) {
            temp = enumerateSubsets(joins, i);
            Iterator<Set<LogicalJoinNode>> it = ((HashSet<Set<LogicalJoinNode>>)temp).iterator();
            while(it.hasNext()) {
                Set<LogicalJoinNode> set = it.next();
                CostCard low = new CostCard();
                low.plan = null;
                low.cost = Integer.MAX_VALUE;
                low.card = Integer.MAX_VALUE;
                Iterator<LogicalJoinNode> iter = ((HashSet<LogicalJoinNode>) set).iterator();
                while(iter.hasNext()) {
                    LogicalJoinNode ljn = iter.next();
                    CostCard cc = computeCostAndCardOfSubplan(stats, filterSelectivities, ljn, set, low.cost, pc);
                    if(cc != null && cc.cost < low.cost) {
                        low = cc;
                    }
                }
                pc.addPlan(set, low.cost, low.card, low.plan);
            }
        }

        Vector<LogicalJoinNode> opt = pc.getOrder(new HashSet<LogicalJoinNode>(this.joins));

        if(explain) {
            printJoins(opt, pc, stats, filterSelectivities);
        }
